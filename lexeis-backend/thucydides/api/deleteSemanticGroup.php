...
$matches = getMatchesList($db, "semantic_lemma_link_plus_lemma", $index);
$occurrences = sizeof($matches);
if ($occurrences > 0) {
  $return = $errorReturn;
  $message = "You cannot delete a semantic group that is linked to a lemma. This semantic group is linked to $occurrences lemmata, including ";
  if (sizeof($matches) == 1) {
...
try {
  ...
  $msg = "Failed to delete semantic group.";
  $query = "UPDATE semantic_groups SET deleted=1 WHERE group_index=?;";
