...
// if stem type  already exists, don't add it
if (getNumMatches($db, "roots", $item) > 0) {
  $return = $errorReturn;
  $return[$errorMessageKey] = "Root $item already exists.";
...
try {
  ...
  $msg = "Failed to add root.";
  $query = "INSERT INTO roots(root, description, lemma_in_dict, deleted) VALUES (?,?,?,0);";
  ...
  changeLogAtomic($db, 24, $stmt->insert_id, "", "");
