...
$matches = getMatchesList($db, "root_lemma_link_plus_lemma", $index);
$occurrences = sizeof($matches);
if ($occurrences > 0) {
  $return = $errorReturn;
  $message = "You cannot delete a root that is linked to a lemma. This root is linked to $occurrences lemmata, including ";
  if (sizeof($matches) == 1) {
...
try {
  ...
  $msg = "Failed to delete root.";
  $query = "UPDATE roots SET deleted=1 WHERE root_index=?;";
